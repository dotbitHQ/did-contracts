...
#[test]
fn challenge_sub_account_approval_revoke_edit_key_not_empty() {
    let mut template = before_each();
...
#[test]
fn challenge_sub_account_approval_revoke_edit_value_not_empty() {
    let mut template = before_each();
...
#[test]
fn challenge_sub_account_approval_revoke_protected_approval() {
    let mut template = init_update();
    ...
    challenge_tx(template.as_json(), SubAccountCellErrorCode::ApprovalInProtectionPeriod)
}
