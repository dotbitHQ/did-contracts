...
#### 创建 DeviceKeyListConfigCell （CreateDeviceKeyList)
...
- 任何人都可以创建 DeviceKeyListConfigCell；
- DeviceKeyListConfigCell 的 das-lock 的 args 中，owner 和 manager 必须相同，等于 witness 中的 DeviceKey；
- input 中 BalanceCell 和 outputs 中 BalanceCell 的 lock 必须一致；
...
#### 更新 DeviceKeyListConfigCell (UpdateDeviceKeyList)
...
- 更新操作包含增加或删除操作；
- 每次只能新增或者删除一个设备；
- UpdateDeviceKeyList 操作中，DeviceKeyList 容量下限是1，容量上限是10；
- 合约允许任何一个已在列表中的 DeviceKey 添加新成员或者删除旧成员；
- 




#### 删除 DeviceKeyListConfigCell (DeleteDeviceKeyList)

当用户选择放弃设备管理时，会清空列表，释放 cell，并把钱回退到 DeviceKeyList 0 号元素的地址；
...
- 退款时，该 cell 占用的 ckb 永远退还到 DeviceKeyList 的第一个元素； 
