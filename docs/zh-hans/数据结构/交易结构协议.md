...
- 拍卖金额以 USD 计算，且必须大于等于 [算法与公式#账户到期后拍卖价格计算公式](./%E7%AE%97%E6%B3%95%E4%B8%8E%E5%85%AC%E5%BC%8F.md#账户到期后拍卖价格计算公式) 中的公式计算结果；
- 拍卖款收款地址需要为 **Wallet Lock** ；
- 输出中 AccountCell 的 owner、manager 地址必须和 ETH 交易中的 data 所指明的收货地址一致，；
- 输出中 AccountCell 的 records 必须被清空；
- 输出中 AccountCell 的到期时间为当前时间之后 1 年；
...
- SubAccountCell 仅能修改 `custom_script` 之后的字段：
  - 如果设置 `custom_script == 255` ，那么就需要：
    - 设置 `price_rules_hash` ，且其值需计算自 witness 中对应的 `SubAccountPriceRule` ，不能为空；
    - 设置 `preserved_rules_hash` ，且其值需计算自 witness 中对应的 `SubAccountPreservedRule` ，可以为空；
  - 如果设置 `custom_script == 0` ，那么可以保留或清空 `price_rules_hash` 和 `preserved_rules_hash`；
