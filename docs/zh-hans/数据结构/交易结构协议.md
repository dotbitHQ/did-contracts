...
- ApplyRegisterCell 必须等待 `ConfigCellApply.apply_max_waiting_block_number` 个区块后才可以进行退款；
...
- PreAccountCell 必须满足以下两个条件之一才可以退款：
  - PreAccountCell 已经创建超过 86400 秒，该值为约定的常量；
  - PreAccountCell 已经创建超过 3600 秒，且 inputs 中有一个 cell 使用了和 PreAccountCell.witness.refund_lock 一致的 lock；
...
> Deprecated! This transaction will be removed in the future.
