...
当账户处于非普通状态时，即 `witness.status != 0` 时，同时账户又到达了生命周期中的宽限期，那么此时 Keeper 可以**强制还原账户的状态**，以便在账户完全到期后进行回收。
当还原时发生销毁 AccountSaleCell, AccountAuctionCell 的情况时，Keeper **必须将这些 Cell 的 capacity 退还给用户**，并可以根据 `ConfigCellAcciont.force_recover_fee` 从 AccountCell 中**取出一部分 capacity 作为手续费**。
...
Inputs:
  ...
  [AccountAuctionCell] // 如果账户 status == 1，那么就必须携带对应的 AccountAuctionCell
  FeeCell
Outputs:
  AccountCell
...
Outputs:
  ...
  [ChangeCell] // AccountSaleCell 的 capacity 必须退还给账户的出售者
```
