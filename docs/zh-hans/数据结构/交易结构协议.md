...
账户**完全到期**后进行回收。当还原时发生销毁 AccountSaleCell, AccountAuctionCell 的情况时，Keeper **必须将这些 Cell 的 capacity 退还给用户**，并
可以根据 `ConfigCellAcciont.common_fee` 从 AccountCell 中**取出一部分 capacity 作为手续费**。
...
table ActionData {
  action: "buy_account",
  params: [new_owner_lock, inviter_lock, channel_lock],
}
```

- new_owner_lock ，购买账户成功后的新的 owner lock，目前必须是一个 das-lock，并且类型不能为 3 ；
