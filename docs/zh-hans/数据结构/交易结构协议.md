...
#### 创建 KeyListConfigCell （CreateKeyList)
...
- 允许任何人为他人创建  KeyListConfigCell；
- 如果产生找零，找零到 FeeCell 的地址；
- 此交易的 witness 中需附带 KeyListConfigCell 中 KeyList 的第一个元素的数据， 合约不做任何校验；



#### 更新 KeyListConfigCell (UpdateKeyList)
...
#### 放弃 KeyListConfigCell (DeleteKeyList)

当用户选择放弃设备管理时，会清空列表，释放 cell，并把钱回退到数组 0 号的设备；
...
A 设备创建 KeyListConfigCell，B、C、D设备加入列表，当选择放弃时， A 设备被移除列表，

可是 KeyListConfigCell 的 lock args 是 A 地址信息。如果退款到 0 号设备，则退款到 B 地址；
