...
#### 创建 DeviceKeyListConfigCell （CreateKeyList)
...
CellDeps:
  das-lock
  ConfigCellMain
  device-key-list-config-cell-type
...
- 允许任何人为他人创建  DeviceKeyListConfigCell；
- 如果产生找零，找零到 FeeCell 的地址；
- 此交易的 witness 中需附带 KeyListConfigCell 中 Key 的数据；
- KeyList 有且仅有一个 Key；
- 合约不会对交易中附带的 KeyList 信息做校验，假如创建错误，则在后续的交易中因无法提供正确签名而验签失败，用户需要重新创建；
- FeeCell: 任何能提供本次交易手续费的Cell；
- ChangeCell: 扣除手续费之后的找零 Cell，其 lock 应该与 FeeCell 相同；




#### 更新 DeviceKeyListConfigCell (UpdateKeyList)

用交易里新的 Witness 替换 keyListConfigCell 里旧的。
...
- 更新操作包含增加或删除操作，但是每次只能新增或者删除一个设备；
- KeyList 上限是10，在最初创建的时候已经添加一个，因此最多再添加9个；
- 交易里带上任何一个已有设备的签名均可添加新设备进来；
- 添加之后立即删除，或者删除之后立即添加，因交易上链顺序不一定，可能的结果不一定满足预期；





#### 放弃 DeviceKeyListConfigCell (DeleteKeyList)

当用户选择放弃设备管理时，会清空列表，释放 cell，并把钱回退到数组 0 号的 payload；

**action 结构**

```
table ActionData {
  action: "delete_device_key_list",
  params: [],
}
```
...
举例说明：当有如下场景时，会退款到第 n 个设备

A 设备创建 DeviceKeyListConfigCell，B、C、D设备加入列表，

* A 设备被移除列表，之后进行 DeleteKeyList 操作，则会退款到 B 地址；

* A 设备被移除列表，之后又添加进来， B 设备被移除列表， 则会退款到 C 地址；
