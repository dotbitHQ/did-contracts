...
- SubAccountCell 的 capacity 包含基础存储费、DAS 的利润、父账户的 owner 利润 3 个部分，因此回收 SubAccountCell 时遵循以下规则退还 capacity：
  - 基础存储费和父账户的 owner 利润退还给父账户的 owner lock；
...
#### 设置子账户创建脚本(ConfigSubAccountCreatingScript)
...
```
table ActionData {
  action: "config_sub_account_creating_script",
...
CellDeps:
  ...
  ConfigCellAccount
  ConfigCellSubAccount
  [AccountCell]  // 如果设置了自定义创建脚本，那么就需要将 AccountCell 移到 cell_deps 中
  [CustomScriptCell] // 如果 SubAccountCell 定义了自定义创建脚本，那么就需要引用该脚本
Inputs:
  [AccountCell]  // 如果设置了自定义创建脚本，那么就需要将 AccountCell 移到 cell_deps 中
  SubAccountCell
  BalanceCell {1,}
Outputs:
  [AccountCell]  // AccountCell 无需任何修改，主要是在未设置自定义创建脚本时用于验签
