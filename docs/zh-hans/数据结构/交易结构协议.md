...
#### 拍卖逾期账户(StartExpiredAccountAuction)

**action 结构**

```
table ActionData {
  action: "start_expired_account_auction",
  params: [],
}
```
...
Inputs:
  AccountCell      // 逾期状态的账户
  FeeCell          // 支付创建 ExpiredAccountAuctionCell 的相关费用
Outputs:
  AccountCell
...
- 当前 TimeCell 必须大于等于 `AccountCell.expired_at + ConfigCellAccount.expiration_grace_period` 才可拍卖；
- 拍卖可以于任意时间开始，并不保证在账户到期后即刻开始；
- 输入中 AccountCell 必须处于 **Normal** 状态，即 `0x00`；
- 输出中 AccountCell 必须处于 **ExpiredAuction** 状态，即 `0x03`；
- 如果存在对应的 SubAccountCell ，必须进行回收；
- 交易手续费由 AccountCell 支付；

#### 成交逾期账户拍卖(AcceptExpiredAccountAuction)

**action 结构**

```
table ActionData {
  action: "accept_expired_account_auction",
  params: [],
}
```
...
Inputs:
  AccountCell       // 逾期状态的账户
  ExpiredAccountAuctionCell
  [SubAccountCell]  // 如果逾期状态的账户已开启子账户，就可以选择在此时回收 SubAccountCell
  BalanceCell       // 支付成交金额
Outputs:
  AccountCell
  BalanceCell       // 转给 DAS lock
  [ChangeCell]
```

**约定**

- 输入中 AccountCell 必须处于 **ExpiredAuction** 状态，即 `0x03`；
- 子账户的回收为可选项；
- 输入中 BalanceCell 的金额必须满足根据当前 TimeCell 计算出来的金额，并且输出中必须转给 DAS ；
- 拍卖结束期限取决于 `ConfigCellAccount.expired_auction_period`;
- 拍卖期结束后并不保证即刻对 AccountCell 进行回收，但根据公式此时售价已经和正常情况下的售价一致；
- 拍卖价的计算公式详见 [计算公式](./计算公式.md)；
...
- 如果账户存在时间已超过拍卖期限 `AccountCell.expired_at + ConfigCellAccount.expiration_grace_period + ConfigCellAccount.expired_auction_period`，仍需将账户转入到期拍卖状态，然后才能进行回收；
...
Inputs:
  AccountCell
  SubAccountCell
  [IncomeCell]   // 当注册费过少时可以用现有的 IncomeCell 来接收子账户注册费
  BalanceCell {1,}
Outputs:
  AccountCell    // AccountCell 无需任何修改，只是用于验签
  SubAccountCell // 子账户的默克尔根必须更新到最终状态 
  [IncomeCell]   // 如果注册费足够可以直接到帐 DAS 官方钱包
  [BalanceCell]
```
...
- 每个账户续费 1 年的金额必须遵循 `ConfigCellSubAccount.renew_sub_account_price + AccountCellData.renew_sub_account_price` 中的配置值；
...
- AccountCell 必须未处于**宽限期**或之后的状态；
- 输入中 AccountCell 必须处于 **Normal** 状态，即 `0x00`；
- 输出中 AccountCell 必须处于 **Bridged** 状态，即 `0x03`；

### 将账户从其他链跨链回来(BridgeFromChain)
...
- 输入中 AccountCell 必须处于 **Bridged** 状态，即 `0x03`；
- 输出中 AccountCell 必须处于 **Normal** 状态，即 `0x00`；

### 申请回收已跨链的账户(ApplyRecyclingBridgedAccount)

当 keeper 或任意角色需要主动回收跨链到其他链的账户时，就可以通过此交易进行申请，然后由多签节点去其他链上尝试进行主动回收。
...
```
table ActionData {
  action: "apply_recycling_bridged_account",
  params: [],
}
```
...
- Action 的 params 参数中需要携带上回收的原因，由多签节点根据回收原因判断账户是否满足回收条件后进行回收，当前可用的回收原因如下：
  - `expired`，账户过期；

### 取消回收已跨链的账户(CancelRecyclingBridgedAccount)

当跨链节点在其他链上尝试回收跨链账户 NFT 失败时，就需要多签节点发出一笔这样的交易作为请求回复。
...
```
table ActionData {
  action: "cancel_recycling_bridged_account",
  params: [],
}
```
...
- Action 的 params 参数中需要携带上回收的原因，由多签节点根据回收原因判断账户是否满足回收条件后进行回收，当前可用回收失败原因如下：
  - `unknown`，未知原因；

### 确认回收已跨链的账户(ConfirmRecyclingBridgedAccount)

当账户在其他链上的 NFT 被成功回收后，就需要多签节点发出一笔这样的交易作为请求回复。

**action 结构**

```
table ActionData {
  action: "confirm_recycling_bridged_account",
  params: [],
}
```
