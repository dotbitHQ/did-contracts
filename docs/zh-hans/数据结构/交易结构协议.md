...
- ReverseRecordCell 的 lock 必须和 inputs[0] 的 FeeCell 一致，如此才能保证反向解析只有拥有对应地址私钥才能声明；
...
- outputs[0] 必须是一个包含了全部 ReverseRecordCell 退款的 Cell；
- 如果 outputs[0] 是 das-lock 且类型为 05 那么必须使用 balance-cell-type 作为 type script，其他情况必须没有任何 type script；
...
Outputs:
  ...
  ChangeCell // AccountSaleCell 的 capacity 必须以使用 das-lock 的 NormalCell 形式退还给账户的出售者
```
