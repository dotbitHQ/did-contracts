...
- input 中 BalanceCell 和 outputs 中 BalanceCell 的 lock需要一致
...
- 退款时，该 cell 占用的 ckb 会退款到device-key-list-cell中制定的refund_lock。 这个refund_lock和创建device-key-list-cell时提供ckb的BalanceCell的lock一致。
