...
- account ，类型 string，代表账户名的 utf-8 字符串，不含后缀部分，比如 `xxxxx.bit` 的 account 变量就是 `xxxxx`，再比如 `xxxxx.yyy.bit` 的 account 变量就是 `xxxxx`；
- account_chars ，类型 string[]，代表账户名的 AccountChars 数据结构，其包含了账户名中每一个字符的所有信息；
