...
```
table SubAccountPreservedRule {
    name: Bytes,
    note: Bytes,
    ast: ASTExpression,
}

vector SubAccountPreservedRules <SubAccountPreservedRule>;
```
...
所有的 operator 执行结束后返回值类型都必须为 `bool` 。
...
```
fn in_list(account: string, root: binary, proof: binary) -> bool;
```
...
{
    "type": "value",
    "value_type": "bool", // bool|unit8|uint32|uint64|binary|string|string[]|charset_type
    "value": {value}
}
```
...
function findPriceRule(priceRules: PriceRule[], account_chars): PriceRule {
    for (let priceRule of priceRules) {
        let val = handleExpression(priceRule.ast, account_chars)
...
let priceRule = findPriceRule(priceRules, accountChars)
