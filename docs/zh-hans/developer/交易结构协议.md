...
将 AccountCell 的 status 从 Normal(0x00) 置为 Upgraded(0x99), 从而创建 DidCell。 本操作只允许对 status 是 Normal 的 AccountCell 进行操作。
...
Inputs:
  AccountCell
  FeeCell // 通过一个 NormalCell 来支付 DidCell 的存储费用
Outputs:
  AccountCell
  DidCell
  [ChangeCell]
...
- 可以在这笔交易交易中直接创建 IncomeCell，IncomeCell 需要满足以下约束：
  - 记录总金额必须和 IncomeCell.capacity 相等；
  - 如果 invitor、channel、DAS 的 lock script 相同，那么他们的利润相关记录必须合并；
  - 其他记录不能和利润相关记录合并；
  - 总记录数必须少于等于 `ConfigCellIncome.max_records`；
...
##### 荷兰式拍卖（BidExpiredAccountDutchAuction）

用户参与荷兰式拍卖时，用户出价 && 发货的交易。

> 荷兰式拍卖是一种特殊的拍卖方式，它的特点是价格从高到低逐渐降低，直到有人出价为止。在这种拍卖方式中，卖方设定一个最高价，然后逐渐降低价格，直到有人出价为止。
> 用户账户在过了宽限期后，进入到拍卖期。
...
本笔交易中，参与的角色有如下几个：
- old owner: 指被拍卖的账户的所有者，账户被拍卖成功后，AccountCell 中的存储费会退回给该用户；
- bidder: 指本次拍卖中出价的人；
- DidSvr : 指由 .bit 团队所拥有的服务地址，用于交易中垫付、收款等。视场景合约可能存在白名单来验证地址；

**约定**

- AccountCell 必须在 Inputs 和 Outputs 的第一个；
- 支付的 DPointCell 的金额应当大于等于拍卖价格，价格的计算规则参见[常量、算法与公式](常量、算法与公式.md)；
- 交易应当发生在拍卖期内，当前时间满足如下约束：
  ```yaml
  expired_time + grace_period <= 当前时间 <=  expired_time + grace_period + auction_period
  ```
    - expired_time 指账户过期时间；
    - grace_period 指账户过期后的宽限期，一般是 90 天；
    - auction_period 指过了保护期后的拍卖期，一般是 30 天；
- Inputs 中的 AccountCell 的属性应该满足如下条件：
  - status 只能是 Normal 或者 LockedForCrossChain；
- Outputs 中的 AccountCell 的属性应该满足如下条件：
  - expired_at 为当前时间 + 1年；
  - records 添加一条默认解析记录；
  - Status 为 Normal；
  - registered_at 为当前时间；
  - last_transfer_account_at、 last_edit_manager_at、last_edit_records_at 为 0；
- Inputs 中，DPointCell 可以是任何人的，包括 old owner，但只能同属于一个用户，不能有多个用户的 DPointCell 来支付拍卖价格；
- Inputs 和 Outputs 中， DidSvr 的 Normal Cell 用来支付或者接收 DPointCell 数量增减带来的 Cell 存储费变化；
- Outputs 中的 BalanceCell 用于退还 AccountCell 的基础存储费；
