...
impl WitnessesParser {
    ...
    pub fn parse_only_action(&mut self) -> Result<(&[u8], &[u8]), Error> {
    ...
    pub fn parse_only_config(&mut self, config_ids: &[ConfigID]) -> Result<&Configs, Error> {
        if self.configs.is_none() {
            ...
            for witness in self.witnesses.iter().skip(1) {
                ...
                match data_type.try_into() {
                    Ok(DataType::ConfigCellMain) => {
                        configs.main = Some(
                            ConfigCellMain::from_slice(entity)
                                .map_err(|_| Error::WitnessDataDecodingError)?,
                        );
                    }
                    Ok(DataType::ConfigCellRegister) => {
                        configs.register = Some(
                            ConfigCellRegister::from_slice(entity)
                                .map_err(|_| Error::WitnessDataDecodingError)?,
                        );
                    }
                    Ok(DataType::ConfigCellBloomFilter) => {
                        configs.bloom_filter = Some(entity.to_vec());
                    }
                    Ok(DataType::ConfigCellMarket) => {
                        configs.market = Some(
    ...
    fn verify_hash_and_get_entity<'a>(
        ...
        let entity = witness
            .get(7..(7 + length))
            .ok_or(Error::ConfigCellWitnessDecodingError)?;
        let hash = blake2b_256(entity).to_vec();
        let ret = config_entity_hashes
            .iter()
            .enumerate()
            .find(|&(_, item)| item.as_slice() == hash.as_slice());
        if let Some((i, _)) = ret {
            config_entity_hashes.remove(i);
        } else {
            return Err(Error::ConfigCellWitnessIsRedundant);
        }
...
mod test {
    ...
    fn before_each() -> WitnessesParser {
        let witnesses = vec![
            ...
            restore_bytes_from_hex(""),
            restore_bytes_from_hex(""),
            restore_bytes_from_hex("0x09750000646173090000000000000000000000000000000000"),
            restore_bytes_from_hex("0x5b0000006461730a000000540000000c000000300000002400000014000000180000001c00000020000000000000000000000080510100e80300002400000014000000180000001c00000020000000008d2700008d270080510100e8030000"),
        ];
    ...
    fn test_parse_only_action() {
        ...
        let (action, params) = parser.parse_only_action().unwrap();
