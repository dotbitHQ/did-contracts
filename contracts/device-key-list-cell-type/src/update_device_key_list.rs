...
use das_types::packed::{DeviceKeyList, DeviceKeyListCellData, DeviceKey};
...
pub fn action() -> Action {
    ...
    update_action.add_verification(Rule::new("Verify capacity change", |contract| {
        assert!(
            contract.input_inner_cells[0].capacity().to_num() - contract.output_inner_cells[0].capacity().to_num() < 10000,
    ...
    update_action.add_verification(Rule::new("Verify key list structure", |contract| {
        ...
        let len_diff: i32 = key_list_in_input.keys().item_count() as i32 - key_list_in_output.keys().item_count() as i32;
        das_core::assert!(
        ...
        match len_diff {
            ...
            -1 => {
                ...
                let keys_in_input: alloc::collections::BTreeSet<Comparable<DeviceKey>> =
                    key_list_in_input.keys().into_iter().map(|key| Comparable(key)).collect();
                let keys_in_output: alloc::collections::BTreeSet<Comparable<DeviceKey>> =
                    key_list_in_output.keys().into_iter().map(|key| Comparable(key)).collect();
                das_core::assert!(
